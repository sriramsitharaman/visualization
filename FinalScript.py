...
def analyze(tweets_data):
    ...
    usStateJson=usState.to_json(orient = "records")
    usStateJsonfinalOutput=usStateJson[33:len(usStateJson)-1].upper().replace("\"STATE\"","ucName").replace("\"VALUE\"","value")
    with open('Final/US_heat_count/usStates-tweetCount-1.json', 'w') as outfile:
        outfile.write(usStateJsonfinalOutput)
    
    #UsStatewise Sentiment
    statesentiout=state_senti(tweetsFinal)
    with open('Final/map-pies/usStates-tweetCount-1.js', 'w') as outfile:
        outfile.write(statesentiout)
    
    #TimeSeries Chart
    timedata=create_timechart(tweets)
    with open('Final/dynamic-master-detail/tweet_cnt-1.js', 'w') as outfile:
        outfile.write(timedata)
    
    #Co-occur Chart
    nodes1,links=co_occur(tweets)
    with open('Final/cooccur_word-1.json', 'w') as outfile:
        outfile.write("{\n"+nodes1+",\n"+links+"}\n")
        
    #Heat World Grid
    heatjson=heatworldgrid(tweets)
